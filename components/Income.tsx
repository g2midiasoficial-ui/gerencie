...
import { ArrowUpCircle, TrendingUp, DollarSign, Calendar, Plus, Trash2, CheckCircle, Copy, RefreshCw, Image as ImageIcon, Music } from 'lucide-react';
...
const Income: React.FC<IncomeProps> = ({ mode }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      for (let i = 0; i < loops; i++) {
          ...
          // Fix JS date rollover issue (e.g. Jan 31 -> Feb 28/29)
  ...
  const handleDuplicate = async (item: Transaction) => {
      if(window.confirm(`Duplicar "${item.description}"?`)) {
          const newItem = await db.transactions.add({
              ...
              date: new Date().toISOString().split('T')[0], // Duplicates to today
              status: 'pending', // Default to pending when duplicating
              type: 'income',
              mode: mode || item.mode // Ensure it stays in current mode
          });
          
          // Optimistic update for immediate feedback
          if (newItem) {
            setIncomes(prev => [newItem, ...prev].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
          }
      }
  };
  ...
  const handleToggleStatus = async (item: Transaction) => {
      ...
      // Update local state immediately for totals calculation
      setIncomes(prev => prev.map(t => t.id === item.id ? { ...t, status: newStatus } : t));
      
      await db.transactions.update(item.id, { status: newStatus });
  };

  const totalReceived = incomes.filter(t => t.status === 'paid').reduce((acc, t) => acc + Number(t.amount), 0);
  const totalPending = incomes.filter(t => t.status === 'pending').reduce((acc, t) => acc + Number(t.amount), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-red-50 border border-red-100 p-6 rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
           <h2 className="text-red-900 font-bold text-xl flex items-center gap-2">
             <ArrowUpCircle className="text-red-600" /> Receitas
           </h2>
           <p className="text-red-700/70 text-sm mt-1">Gerencie suas fontes de renda e entradas.</p>
        </div>
        <button 
            onClick={() => { resetForm(); setIsModalOpen(true); }}
            className="w-full sm:w-auto bg-green-600 text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-green-700 transition-colors shadow-lg shadow-green-200 flex items-center justify-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">Recebido este Mês</p>
              <h3 className="text-2xl font-bold text-gray-800 mt-2">R$ {totalReceived.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
          </div>
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">Pendente</p>
              <h3 className="text-2xl font-bold text-orange-500 mt-2">R$ {totalPending.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
          </div>
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">Previsão Total</p>
              <h3 className="text-2xl font-bold text-green-600 mt-2">R$ {(totalReceived + totalPending).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
          </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
         <div className="p-6 border-b border-gray-100">
            <h3 className="font-bold text-gray-800">Últimas Entradas</h3>
         </div>
                    ...
                    {incomes.map(item => (
                        <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="px-6 py-4">
                                <span className="px-2 py-1 bg-green-50 text-green-700 border border-green-100 text-xs rounded-md font-medium">{item.category}</span>
                            </td>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
                ...
                <div className="relative">
                    <select 
                        required
                        className="w-full p-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all bg-white appearance-none"
                        value={category}
            ...
            <div>
                ...
                <div className="flex gap-4">
                    <label className={`flex-1 cursor-pointer border rounded-xl p-3 flex items-center justify-center gap-2 transition-all ${status === 'paid' ? 'bg-green-50 border-green-200 text-green-700 ring-1 ring-green-200' : 'hover:bg-gray-50 border-gray-200'}`}>
                        ...
                        <span className="w-2.5 h-2.5 rounded-full bg-green-500"></span> Recebido
                    </label>
                    <label className={`flex-1 cursor-pointer border rounded-xl p-3 flex items-center justify-center gap-2 transition-all ${status === 'pending' ? 'bg-orange-50 border-orange-200 text-orange-700 ring-1 ring-orange-200' : 'hover:bg-gray-50 border-gray-200'}`}>
                        <input type="radio" name="status" value="pending" checked={status === 'pending'} onChange={() => setStatus('pending')} className="hidden"/>
                        <span className="w-2.5 h-2.5 rounded-full bg-orange-500"></span> Pendente
                    </label>
                </div>
            </div>
            ...
            <button 
                type="submit"
                className="w-full bg-green-600 text-white font-bold py-3.5 rounded-xl hover:bg-green-700 transition-all shadow-lg shadow-green-100 mt-2"
            >
                Salvar Receita
            </button>
        </form>
